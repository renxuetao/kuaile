package com.klzb.live.home.adapter;
...
public class HotRecyclerListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof VideoViewHolder) {
            ...
            FunctionUtile.setLevel(mContext,videoViewHolder.mTextUserLevel,level);
//            if(level<5){
...
//            }
            ...
            if(StringUtils.isNotEmpty(item.getMinute_charge()) && Integer.parseInt(item.getMinute_charge()) > 0){
                videoViewHolder.mTextLiveStatus.setText(item.getMinute_charge()+"/分钟");
            }
