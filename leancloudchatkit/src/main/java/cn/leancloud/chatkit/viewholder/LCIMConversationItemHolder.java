...
import cn.leancloud.chatkit.utils.PicassoCircleTransform;
...
public class LCIMConversationItemHolder extends LCIMCommonViewHolder {
    ...
    public void initView() {
        itemContainer = (LinearLayout) itemView.findViewById(R.id.linear_conversation_item_container);
    ...
    private void updateLastMessage(AVIMMessage message) {
        if (null != message) {
            ...
            CharSequence messageeShorthand = getMessageeShorthand(getContext(), message);
            Log.i("updateLastMessage", "updateLastMessage==messageeShorthand=" + messageeShorthand);
            try {
                JSON.parseObject(messageeShorthand.toString());
                Log.i("updateLastMessage", "updateLastMessage==josnObject=");
                ViewGroup.LayoutParams params = itemContainer.getLayoutParams();
                params.height = 0;
                itemContainer.setLayoutParams(params);
                itemContainer.setVisibility(View.GONE);
            }catch (Exception e){
                Log.i("updateLastMessage", "updateLastMessage==not jsonObject=");
                if (TextUtils.isEmpty(messageeShorthand)){
                    ViewGroup.LayoutParams params = itemContainer.getLayoutParams();
                    params.height = 0;
                    itemContainer.setLayoutParams(params);
                    itemContainer.setVisibility(View.GONE);
